...
const GAME_OPEN_AT = new Date("2026-05-15T09:00:00+09:00").getTime();
...
export default function Home() {
  ...
  const buyStock = (stock: Stock) => {
    if (!isGameOpen) {
      alert("어인섬 Fish Coin 거래소는 5월 15 오전 9시에 오픈합니다.");
      return;
    }
    if (!isMarketOpen) {
      alert("현재 장 마감 상태입니다.");
  ...
  const sellStock = (stock: Stock) => {
    if (!isGameOpen) {
      alert("어인섬 Fish Coin 거래소는 5월 15일 오전 9시에 오픈합니다.");
      return;
    }
    if (!isMarketOpen) {
      alert("현재 장 마감 상태입니다.");
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-end mb-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl px-4 py-2 text-sm text-zinc-300">
            {!isGameOpen ? "5월 15일 오전 9시 오픈 예정" : isMarketOpen ? (
